...
program
  ...
  .action(async (component) => {
    ...
    const componentDir = path.join(process.cwd(), 'components', 'ui');
    const componentPath = path.join(componentDir, `${component}.tsx`); //! Changed to .js because tsx or jsx not working because build of dist contains only tsx and jsx
    
    // Adjusted paths
    const libComponentDir = path.join(__dirname, '..', 'components');
    const libComponentPath = path.join(libComponentDir, component, `${component}.tsx`); //! Changed to .js because tsx or jsx not working because build of dist contains only tsx and jsx
